	...
	switch(shoot->info->shoot_work_info->shoot_work_status)
	{
		...
		case S_Shoot:
		  ...
		  if(shoot->info->shoot_work_info->shoot_work_cnt >= shoot->config->position_work_time)
			...
			break;
		case S_Wake:
			shoot->info->fric_speed_target = shoot->config->fric_speed_tar;
		...
		case S_Standby:
			switch(shoot->info->shoot_work_info->shoot_work_command)
			{
				...
				case S_Wake:
				  ...
				  break;
				case S_Shoot:
					if(shoot->info->shoot_work_info->standby_yes == 1 && shoot->info->shoot_work_info->feed_work_info->feed_work_status == F_static)
					{
						if(judge.base_info->shooter_cooling_heat <= judge.base_info->shooter_cooling_limit - 100)
						{
							...
							judge.base_info->shooter_cooling_limit = 0;
						}
					}
					else
					{
...
//			tested = judge.base_info->shooter_cooling_heat;
			break;
		case S_Done:
