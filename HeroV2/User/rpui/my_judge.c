...
uint8_t global_vision_on = 0;
uint8_t global_detect_top = 0;
...
void global_vary_system(void)
{
	...
	global_vision_on = global_receive.vision_on;
	
	global_detect_top = global_receive.detect_top;
...
void UI_send_Group4(void)
{
  ...
  else 
  {
	...
	if(global_detect_top == 1)
	{
		vision_color = FUCHSIA;
	}
	else
	{
		if(global_vision_on == 1)
		{
			vision_color = GREEN;
		}
		else
