...
void Chassis_Process(Chassis_Mode_t* chassis_mode)
{
	...
	switch(chassis_mode->chassis_move_mode)
	{
		...
		case KEY_CTRL:
		    ...
		    {
			    Chassis.base_info.target.front_speed = (front * 1.4f);
		      Chassis.base_info.target.right_speed = (right * 1.4f);
  		  }
			  else
			  {
	...
	switch(chassis_mode->chassis_spin_mode)
	{
		...
		case C_S_follow:
			...
			{
				Chassis.base_info.target.cycle_speed = 7 * err_yaw_zero - sgn(err_yaw_zero) * 350;
			}
			else
			{
				Chassis.base_info.target.cycle_speed = 3.5f * err_yaw_zero;
			}
		...
		case C_S_top:
			switch(Chassis_Config.top_mode)
			{
				case 0:
				  ...
				  if(chas_speed_up == true)
					{
						Chassis.base_info.target.cycle_speed *= 2;
					}
